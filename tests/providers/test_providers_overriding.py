...
async def test_batch_providers_overriding() -> None:
    async_resource_mock: datetime.datetime = datetime.datetime.fromisoformat("2023-01-01")
    sync_resource_mock: datetime.datetime = datetime.datetime.fromisoformat("2024-01-01")
    async_factory_mock: datetime.datetime = datetime.datetime.fromisoformat("2025-01-01")
    simple_factory_mock: container.SimpleFactory = container.SimpleFactory(dep1="override", dep2=999)
    singleton_mock: container.SingletonFactory = container.SingletonFactory(dep1=False)

    providers_for_overriding: typing.Dict[str, typing.Any] = {
...
async def test_batch_providers_overriding_sync_resolve() -> None:
    ...
    with container.DIContainer.override_providers(providers_for_overriding):
        await container.DIContainer.simple_factory.sync_resolve()
        await container.DIContainer.async_resource.async_resolve()
        dependent_factory = await container.DIContainer.dependent_factory.sync_resolve()
        singleton = await container.DIContainer.singleton.sync_resolve()
    ...
    assert (await container.DIContainer.sync_resource.sync_resolve()) != sync_resource_mock
...
def test_providers_overriding_fail_with_unknown_provider() -> None:
    unknown_provider_name: str = "unknown_provider_name"
    match: str = f"Provider with name {unknown_provider_name!r} not found"
    providers_for_overriding: typing.Dict[str, typing.Any] = {unknown_provider_name: None}
