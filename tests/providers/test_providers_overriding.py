...
async def test_batch_providers_overriding() -> None:
    ...
    # Verify that overrides are reset after context exit
...
def test_providers_overriding_with_context_manager() -> None:
    ...
    with container.DIContainer.simple_factory.override_context(simple_factory_mock):
        resolved_simple_factory = container.DIContainer.simple_factory.sync_resolve()
        assert resolved_simple_factory is simple_factory_mock

    # Verify that the override is reset after the context manager exits
    resolved_simple_factory = container.DIContainer.simple_factory.sync_resolve()
    assert resolved_simple_factory is not simple_factory_mock
...
async def test_providers_overriding() -> None:
    ...
    # Override providers individually
