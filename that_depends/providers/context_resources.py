...
class container_context(  # noqa: N801
    ...
    def __exit__(
        ...
        try:
            self._tear_down_resources(is_async=False)
        finally:
    ...
    async def __aexit__(
        ...
        if self._context_token is None:
            ...
            raise RuntimeError(msg)
        try:
            await self._tear_down_resources(is_async=True)
        finally:
            _CONTAINER_CONTEXT.reset(self._context_token)
    ...
    def _tear_down_resources(self, is_async: bool) -> None:
        context = _CONTAINER_CONTEXT.get()
        for context_item in reversed(context.values()):
            if isinstance(context_item, ResourceContext):
                if is_async and context_item.is_async:
                    await context_item.tear_down()
                elif not is_async and not context_item.is_async:
